...
def makeLines():
    pointCount = 256
    hLineLR = Gesture([(-t, 0) for t in range(pointCount)],
                    name="Horizontal Line Right to Left")
    hLineRL = Gesture([(t, 0) for t in range(pointCount)],
                    name="Horizontal Line Left to Right")
    vLineTB = Gesture([(0, t) for t in range(pointCount)],
                    name="Vertical Line Top to Bottom")
    vLineBT = Gesture([(0, -t) for t in range(pointCount)],
                    name="Vertical Line Bottom to Top")
    diagonalTLtoBR = Gesture([( t,  t) for t in range(pointCount)],
                            name="Diagonal Top Left to Bottom Right")
    diagonalBRtoTL = Gesture([(-t, -t) for t in range(pointCount)],
                            name="Diagonal Bottom Right to Top Left")
    diagonalTRtoBL = Gesture([(-t,  t) for t in range(pointCount)],
                            name="Diagonal Top Right to Bottom Left")
    diagonalBLtoTR = Gesture([( t, -t) for t in range(pointCount)],
